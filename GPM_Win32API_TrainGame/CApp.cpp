...
bool CApp::Init(HINSTANCE hInstance, int nCmdShow, POINT _pResolution)
{
	...
	// Bitmap
	m_bitmap.Init();
	m_bitmap.Load(m_hDC, L"Road.bmp");
	m_hBmpDC = m_bitmap.GetBmpDC();


	//m_bitmap.ReadBitmap(m_hDC, m_hMemDC, L"Road.bmp");
	//BitBlt(m_hDC, 0, 0, WIDTH, HEIGHT, m_hMemDC, 0, 0, SRCCOPY
	return true;
}
